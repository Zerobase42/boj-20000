last_row=
def solve3(sx,sy,c,l):
    global b
    if c=='/':rs,re=sx-1024,sx+1
    else:rs=sx;re=re+1024-sy
    stop=1
    for i in range(rs,re):
        if (last_row>>i)&1:stop=0
    if stop:return
    rec=1
    if not l:l=1;rec=0
    dx=-1 if c=='/'else 1
    dy=1
    for i in range(len):
        x=sx+dx*i;y=sy+dy*i
        if 0<=x<2048 and 0<=y<1024:b[y][x]=c
    if not rec:return
    solve3(sx+dx*l,sy+dy*l,c,l/2)
    solve3(sx+dx*(l-1),sy+dy*(l-1)+1,'\\'if c=='/'else'/',l/2)

n=int(input())
if n==0:print('BOJ 20000')
if n==1:
    s='''#include <cstdio>
int main(){
    int N;
    scanf("%d",&N);
'''
    for i in range(1,20000):s+=f""" if(N=={i}){{
        puts("{i*4}");
    }}
    else"""
    print(s+"""{
        puts("Still working on it...");
    }
    return 0;
}""")
if n==2:
    p=['','BaBeBaBkBaBeBaB','jojojojnjojojoj','OnOlOnO','inieini','JuJ']
    r=''
    for i in range(4,0,-1):
        t=''
        for c in r:t+=p[i-1]+c
        r+=t+p[i-1]
    for c in r:print(end=p[i-1]+c)
    print(p[1])
if n==3:
    b=[[' ']*2048 for _ in range(2048)]
    solve3(1023,0,'/',512);solve3(1024,0,'\\',512)
    for j in range(1024):
        s=''
        for i in range(2048):
            if i<1025+j:s+=b[j][i]
        print(s)
if n==4:pass
if n==5:print(""*200)